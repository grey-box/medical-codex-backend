...
prepared_data = {
    ...
    "fda_product_code_classification": pd.read_csv(
        prepared_data_path / "fda_product_code_classification.csv"
    ),
...
translation_data_compendium = (
    ...
    .drop_duplicates()
    .merge(
        prepared_data["compendium_ru"][
            ["title_latin_name", "title_ru_name"]
        ].drop_duplicates(),
        how="outer",
        on="title_latin_name",
        suffixes=("_uk", "_ru"),
    )
    .assign(source="compendium.com.ua")
    .reset_index(drop=True)
    .rename(columns={"title_latin_name": "medicine_name"})
)

## %% WHO Essential
translation_data_compendium["medicine_name_who_matches"] = translation_data_compendium[
    "medicine_name"
].apply(
    lambda x: close_match_substring(
        x, list(prepared_data["who_essential"]["medicine_name"]), cutoff=0.6
    )
)

## %% Merge compendium and WHO data
translation_data_who = translation_data_compendium.merge(
    prepared_data["who_essential"][
        ["medicine_name", "atc_classification", "source"]
    ].drop_duplicates(),
    how="outer",
...
translation_data_who["medicine_name_merge1_en"] = translation_data_who[
    ["medicine_name_who_matches", "medicine_name_who"]
].apply(
    lambda x: (
        x["medicine_name_who_matches"]
        if pd.isna(x["medicine_name_who"])
        else x["medicine_name_who"]
    ),
    axis=1,
)
...
wikidata_labels_aliases = pd.concat(
    [
        # Labels only
        prepared_data["wikidata_names"][
            ["id", "label_uk", "label_ru", "label_en"]
        ].rename(columns={"id": "wikidata_id"}),
...
wikidata_labels_aliases = wikidata_labels_aliases.dropna(
    subset=["label_uk", "label_ru", "label_en"]
).reset_index(drop=True)

## %% merge wikidata names with Compendium and WHO data
translation_data_wikidata = wikidata_labels_aliases[
    ["wikidata_id", "label_uk", "label_ru", "label_en"]
].merge(
...
## %% assign first non-empty value to medicine_name column
translation_data_wikidata["medicine_name_merge2_en"] = translation_data_wikidata[
    ["medicine_name_merge1_en", "label_en"]
].apply(
    lambda x: (
        x["label_en"]
        if pd.isna(x["medicine_name_merge1_en"])
        else x["medicine_name_merge1_en"]
    ),
    axis=1,
)

translation_data_wikidata["medicine_name_merge2_uk"] = translation_data_wikidata[
    ["title_uk_name", "label_uk"]
].apply(
    lambda x: x["label_uk"] if pd.isna(x["title_uk_name"]) else x["title_uk_name"],
    axis=1,
)

translation_data_wikidata["medicine_name_merge2_ru"] = translation_data_wikidata[
    ["title_ru_name", "label_ru"]
].apply(
...
drugbank_data = (
    prepared_data["drugbank_vocabulary"][["common_name"]].drop_duplicates().copy()
)
# %%
drugbank_data["best_match_medicine_name_merge2_en"] = drugbank_data[
    "common_name"
].apply(
    lambda x: close_match_substring(
        str(x),
...
drugbank_data_synonyms = (
    prepared_data["drugbank_vocabulary"][["synonyms"]].drop_duplicates().copy()
)
## %%
drugbank_data_synonyms["best_match_synonym_merge2_en"] = drugbank_data_synonyms[
    "synonyms"
].apply(
    lambda x: close_match_substring(
        str(x),
...
translation_data_drugbank2 = translation_data_drugbank.merge(
    prepared_data["drugbank_vocabulary"][
        ["common_name", "drugbank_id"]
    ].drop_duplicates(),
    how="outer",
    left_on="common_name",
...
## %% Best english name
translation_data_drugbank2["medicine_name_merge3_en"] = translation_data_drugbank2[
    ["medicine_name_merge2_en", "common_name"]
].apply(
    lambda x: (
        x["common_name"]
        if pd.isna(x["medicine_name_merge2_en"])
        else x["medicine_name_merge2_en"]
    ),
    axis=1,
)
...
single_table_data = (
    ...
    .rename(
        columns={
            "medicine_name_merge3_en": "English Name",
...
single_table_data.to_excel(
    pathlib.Path(os.getcwd())
    / "merged_data"
    / "translation_data_wikidata_2024-07-14.xlsx",
    index=False,
)
